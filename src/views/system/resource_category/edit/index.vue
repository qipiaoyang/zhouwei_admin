...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState({
        editVisible: state => state.resource_category.editVisible,
        datainfo: state => state.resource_category.datainfo,
      }),
    },
    methods: {
      cancelDialog() {
        this.$store.commit("resource_category/SET_EDITVISIBLE", false);
      },
      updateData() {
        let that = this;
        this.$refs['dataForm'].validate((valid) => {
          if(valid) {
            this.$store.dispatch("resource_category/updateResourceCategory", that.datainfo).then((e) => {
              if(e.success) {
                that.$notify({
                  title: '编辑角色成功',
                  type: 'success',
