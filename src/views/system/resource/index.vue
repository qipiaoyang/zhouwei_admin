...
<script>
  ...
  export default {
    ...
    created() {
      this.$store.dispatch("resource/getResourceList");
    },
    computed: {
      ...mapState({
        token: state => state.resource.token,
        resourceList: state => state.resource.resourceList,
        listQuery: state => state.resource.listQuery,
        total: state => state.resource.total,
        listLoading: state => state.resource.listLoading,
      })
    },
    methods: {
      // 重置功能
      resetList() {
        this.$store.commit("resource/RESET_LISTQUERY")
        this.$store.dispatch("resource/getResourceList");
      },
      // 获取列表
      getList() {
      ...
      // 创建角色
      handleCreate() {
        this.$store.commit("resource/SET_ADDVISIBLE", true);
      },
      //编辑角色
      handleUpdate(row) {
        this.$store.dispatch("resource/getResourceInfo", row.id);
      },
      // 启用禁用
      handleVisible(row) {
        var that = this;
        this.$store.commit("resource/SET_ID", row.id);
        this.$store.dispatch("resource/changeVisibleResource", {status: row.status ? 0 : 1}).then((e) => {
          if(e.success) {
            that.$notify({
      ...
      handleFilter() {
        ...
        this.$store.commit("resource/SET_LISTQUERY", this.listQuery)
        this.$store.dispatch("resource/getResourceList");
      },
      sortChange(data) {
