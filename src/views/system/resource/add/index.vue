...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState({
        addVisible: state => state.resource.addVisible,
      }),
    },
    methods: {
      cancelDialog() {
        this.$store.commit("resource/SET_ADDVISIBLE", false);
      },
      createData() {
        let that = this;
        this.$refs['dataForm'].validate((valid) => {
          if(valid) {
            this.$store.dispatch("resource/createResource", this.formObj).then((e) => {
              if(e.success) {
                that.$notify({
                  title: '添加用户成功',
                  type: 'success',
